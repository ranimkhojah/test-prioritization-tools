...
do
  ...
  awk -v projectname=$projectid -v fileincrement=$fileincrement ' BEGIN {
    FS = ",";
    # print " File: " ARGV[2];
...
# inside rankings
  NR!=FNR {
    ...
    if (FNR == 1) {printf $0 ",Total_Faults" > "./rankedtotalfaults/" currentfilename "_totalfaults_" fileincrement ".csv";}
    if (FNR != 1) {
      # print FNR "   --   ";
      printf "\n" $0 >> "./rankedtotalfaults/" currentfilename "_totalfaults_" fileincrement ".csv";
      ...
      if (currentcounter != prevcounter) {
        ...
        # 0 here, but if same line is trigger it will +1 below - reset everything
        faults = 0;
        cappedfaults = 0;
        failures = 0;
      }
      ...
      printf "," cappedfaults >> "./rankedtotalfaults/" currentfilename "_totalfaults_" fileincrement ".csv";
    }
    prevcounter = currentcounter;
  }
...
END {
  if (failures < triggerslength && failures > 0) {
    ...
    print currentfile " - Found Failures: " failures >> "./rankedtotalfaults/" projectname "_totalfaults_log.txt";
    for (i = 1; i < faults + 1; i++) {
      print i " -> " foundtriggersarr[i];
      print i " -> " foundtriggersarr[i] >> "./rankedtotalfaults/" projectname "_totalfaults_log.txt";
    }
  }
  if (failures == triggerslength) {
  ...
  if (failures == 0) {
    ...
    print currentfile "     >>>>>>>> NO TRIGGERS FOUND" >> "./rankedtotalfaults/" projectname "_totalfaults_log.txt";
  }
}
