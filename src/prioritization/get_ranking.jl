...
function MaxiMinDiversitySequence(filename)
    # strings = String[string(o) for o in objects]
    # df = CSV.read("d2v_p1.csv")
    # dm = Matrix{Float64}(df)
    dm = readdlm(filename,';', Float64)
...
function main(pargs)
    ...
    @time begin
        ...
        # filename = "distances_Jaccard_2_20230113_123809.csv"
        # seq = MaxiMinDiversitySequence(filename)
        # rankvec = ranks(seq)
        # ma = zeros(Float64, length(rankvec), 1)
        # ma[:, 1] = rankvec
        # println(ma)
